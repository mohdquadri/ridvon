...
export function MoversBar() {
  ...
  return (
    <div className="flex items-center gap-4 overflow-hidden border-b border-border bg-surface px-5 py-3 sm:px-7">
      <span className="shrink-0 border-r border-border pr-4 text-[11px] font-semibold uppercase tracking-wider text-muted">S&P 500 Movers</span>
      <div className="flex min-w-0 flex-1 items-center gap-5 overflow-x-auto ticker-scroll">
        {isLoading && mixed.length === 0 ? (
          ...
          <button key={q.symbol} type="button" onClick={() => void navigate({ to: "/analysis", search: { ticker: q.symbol, tab: "analyze" } })} className="flex shrink-0 items-center gap-1.5 whitespace-nowrap">
            <span className="text-[13px] font-semibold text-fg">{q.symbol}</span>
            <span className={cn("text-[13px] font-semibold tabular", q.changePercent >= 0 ? "text-gain" : "text-loss")}>{formatPercent(q.changePercent)}</span>
          </button>
        ))}
      </div>
    </div>
